...
# Skip tests that need the real library when it is not installed
_NGSPICE_AVAILABLE = ctypes.util.find_library("ngspice") is not None
_needs_lib = pytest.mark.skipif(
    not _NGSPICE_AVAILABLE, reason="libngspice not installed on this system"
)
